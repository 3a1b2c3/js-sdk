...
- **events** — detail clauses. **Character** events (`actor: "character"`, the default) stack while their hold-key is held; **environment** events (`actor: "environment"`) are instead asserted as persistent facts in the coordinator's shared History and projected onto the prompt until cleared
...
| `actor`                                        | `"character"` (default) → hold-key 1–9 character action · `"environment"` → world event fired from the Director panel / letter hotkey. |
...
The authoring **rules** (character/environment split, "exactly one landmark at a fixed position," destroyed things vanish, visual-only/no sound, camera matches the image) live in the `/add-game` skill (`.claude/skills/add-game/SKILL.md`).
...
| [`components/lingbot-world-2/LayeredSceneEditor.tsx`](components/lingbot-world-2/LayeredSceneEditor.tsx)         | The full-screen layered prompt editor behind every ✎ button. Per-event: character/environment toggle (color-coded), version pickers, and the signed cost (`health Δ`) field.                                    |
| [`lib/DirectorPanel.tsx`](../../lib/DirectorPanel.tsx)                   | In-app Human Director — a second coordinator client (role `human`). Per-game SCENE buttons (with their letter hotkeys), the human/ai/both switch, and free-form assert/clear.                             |
